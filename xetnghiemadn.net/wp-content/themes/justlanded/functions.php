 ...
 * (C) Copyright 2012-2014 ShapingRain
 *
 * License:		GPLv2
 *
 ...
 * Other Libraries
 */
require_once ( JUSTLANDED_MAIN_DIR . 'libs' . DIRECTORY_SEPARATOR . 'cssmin.class.php' );
...
if (!function_exists('justlanded_get_option')){
	function justlanded_get_option($key, $default = "", $overwrite_data = null) {
		// use global or custom data array?
		if (!is_null($overwrite_data)) {
			$data = $overwrite_data;
		} else {
			global $data; // get theme options from global data array
		}
...
// load definitions for shortcodes and tinymce buttons
global $wp_version;
$tiny_mce_suffix = "";
if( version_compare( $wp_version, '3.8', '>') ) {
	$tiny_mce_suffix = "-tinymce4";
}
$t = new add_new_tinymce_btn('','justlanded', get_bloginfo('template_url').'/shortcodes/' . 'shortcode-justlanded' . $tiny_mce_suffix . '.js');
require(JUSTLANDED_SHORTCODES_DIR . 'shortcode-justlanded.php');

function justlanded_admin_url () { ?>
	<script type="text/javascript" language="javascript">
		var justlanded_theme_base_url = '<?php echo get_template_directory_uri(); ?>';
	</script>
<?php }
add_action( 'admin_head', 'justlanded_admin_url' );


// add custom CSS
add_action('wp_enqueue_scripts', 'justlanded_enqueue_custom_stylesheet', 20);
...
add_filter('stylesheet_uri','justlanded_stylesheet_uri',10,2);
if (!function_exists('justlanded_stylesheet_uri')) {
	function justlanded_stylesheet_uri($stylesheet_uri, $stylesheet_dir_uri){
		global $data;

		if ( justlanded_get_option('disable_responsiveness', false ) == true ) {
			return $stylesheet_dir_uri . '/style-static.css' ;
		}
		else {
...
if(!function_exists('justlanded_dynamic_css')){
	function justlanded_dynamic_css() {
		...
		// dynamic css compression
		if (!defined('JUSTLANDED_NO_CSS_COMPRESSION')) {
		...
		echo '</script>'."\n";
		echo '<script src="'.get_stylesheet_directory_uri().'/scripts/css3-mediaqueries.min.js"></script>'."\n";
		echo '<![endif]-->'."\n";
...
if(!function_exists('justlanded_additional_scripts_footer')){
	function justlanded_additional_scripts_footer() {
		...
		if (isset($data['testimonial_slider_animation_type']))  $testimonial_slider_animation_type  = $data['testimonial_slider_animation_type'];          else $testimonial_slider_animation_type  = "fade";
		if (isset($data['testimonial_slider_slideshow_speed'])) $testimonial_slider_slideshow_speed = intval($data['testimonial_slider_slideshow_speed']); else $testimonial_slider_slideshow_speed = "7000";
		if (isset($data['testimonial_slider_animation_speed'])) $testimonial_slider_animation_speed = intval($data['testimonial_slider_animation_speed']); else $testimonial_slider_animation_speed = "600";
		if (isset($data['testimonial_slider_random']))          $testimonial_slider_random          = $data['testimonial_slider_random'];                  else $testimonial_slider_random           = "false";
		if (isset($data['testimonial_slider_controls']))        $testimonial_slider_controls        = $data['testimonial_slider_controls'];                else $testimonial_slider_controls         = "true";

		if ( justlanded_get_option('lightbox_active_for_links', false) == true ) {
		...
		if ( justlanded_get_option('lightbox_active_for_gallery', true) == true ) {
        ...
        ;(function ($, window, undefined) {
            $(document).ready(function() {
                ...
                $('.testimonial-slider-large').flexslider({
                    animation: '".$testimonial_slider_animation_type."',
                    slideshowSpeed: ".$testimonial_slider_slideshow_speed.",
                    animationSpeed: ".$testimonial_slider_animation_speed.",
                    randomize: ".$testimonial_slider_random.",
                    controlNav: false,
                    directionNav: ".$testimonial_slider_controls."
                });
...
if(!function_exists('justlanded_scripts_load_cdn')){
	...
	{
		wp_register_script('justlanded-slider', get_template_directory_uri() . '/scripts/jquery.flexslider.min.js', array( 'jquery'), THEME_VERSION, true);
		wp_enqueue_script('justlanded-slider');

		wp_register_script('justlanded-modernizr', get_template_directory_uri() . '/scripts/custom.modernizr.min.js', array( 'jquery' ), THEME_VERSION, true);
		wp_enqueue_script('justlanded-modernizr');

		wp_register_script('justlanded-foundation', get_template_directory_uri() . '/scripts/foundation.custom.min.js', array( 'jquery', 'justlanded-modernizr' ), THEME_VERSION, true);
		wp_enqueue_script('justlanded-foundation');

		wp_register_script('justlanded-touchtouch', get_template_directory_uri() . '/scripts/touchtouch.jquery.min.js', array( 'jquery' ), THEME_VERSION, true);
		wp_enqueue_script('justlanded-touchtouch');

		wp_register_script('justlanded-tinynav', get_template_directory_uri() . '/scripts/tinynav.min.js', array( 'jquery' ), THEME_VERSION, true);
		wp_enqueue_script('justlanded-tinynav');

		/*
		wp_register_script('justlanded-waypoints', get_template_directory_uri() . '/scripts/waypoints.custom.min.js', array( 'jquery' ), THEME_VERSION, true);
		wp_enqueue_script('justlanded-waypoints');
		*/

		wp_register_script('justlanded-site-script', get_template_directory_uri() . '/scripts/site.js', array( 'jquery', 'justlanded-modernizr', 'justlanded-foundation', 'justlanded-touchtouch', 'justlanded-tinynav', 'justlanded-slider' ), THEME_VERSION, true);
...
if (!function_exists('justlanded_add_body_classes')) {
	function justlanded_add_body_classes($classes) {
		global $data;
		if (isset($data['sticky_menu']) && $data['sticky_menu'] == 1 ) $classes[] = 'has-sticky-menu'; // sticky menu
		return $classes;
	}
}
add_filter('body_class','justlanded_add_body_classes', 999);
...
if ( ! function_exists( 'justlanded_comment_form' ) ) :
	function justlanded_comment_form() {
		...
		$args = array(
			...
			'title_reply'       => __( 'Leave a Reply', 'justlanded' ),
			'title_reply_to'    => __( 'Leave a Reply to %s', 'justlanded' ),
			'cancel_reply_link' => __( 'Cancel Reply', 'justlanded' ),
			'label_submit'      => __( 'Post Comment', 'justlanded' ),

			'comment_field' =>  '<p class="comment-form-comment">' .
			                    '<textarea id="comment" name="comment" cols="45" placeholder="'._x( 'Comment', 'noun', 'justlanded' ).'" rows="8" aria-required="true">' .
			                    '</textarea></p>',

			'must_log_in' => '<p class="must-log-in">' .
			                 sprintf(
				                 __( 'You must be <a href="%s">logged in</a> to post a comment.', 'justlanded' ),
				                 wp_login_url( apply_filters( 'the_permalink', get_permalink() ) )
			                 ) . '</p>',

			'logged_in_as' => '<p class="logged-in-as">' .
			                  sprintf(
				                  __( 'Logged in as <a href="%1$s">%2$s</a>. <a href="%3$s" title="Log out of this account">Log out?</a>', 'justlanded' ),
			...
			'comment_notes_after' => '<p class="comment-notes">' .
			                         __( 'Your email address will not be published.', 'justlanded' ) .
			                         '</p>',
			...
			'fields' => apply_filters( 'comment_form_default_fields', array(
					'author' =>
						...
						'<input id="author" name="author" type="text" placeholder="'.__( 'Name', 'justlanded' ).'" value="' . esc_attr( $commenter['comment_author'] ) .
						'" size="30"' . $aria_req . ' /></p>',

					'email' =>
						'<p class="comment-form-email">' .
						'<input id="email" name="email" type="text" placeholder="'.__( 'Email', 'justlanded' ).'" value="' . esc_attr(  $commenter['comment_author_email'] ) .
						'" size="30"' . $aria_req . ' /></p>',

					'url' =>
						'<p class="comment-form-url">' .
						'<input id="url" name="url" type="text" placeholder="'.__( 'Website', 'justlanded' ).'" value="' . esc_attr( $commenter['comment_author_url'] ) .
						'" size="30" /></p>'
				)
			),
		);
...
if (!function_exists('justlanded_add_classes_to_widgets')) {
	function justlanded_add_classes_to_widgets($params){
		...
		if (isset($data['widgets_columns'])) $max_columns = $data['widgets_columns'];
		if ($max_columns == 4) $column_class = "one_fourth";
		if ($max_columns == 3) $column_class = "one_third";
		if ($max_columns == 2) $column_class = "one_half";
		...
		if ($params[0]['id'] == "landing-page-widgets-row-1"){
			...
			if ($justlanded_widgets_row_count[1] == $max_columns) {
				$before_widget = str_replace('class="', 'class="widget-container '.$column_class.' last ', $before_widget);
				$params[0]['before_widget'] = $before_widget;
				$justlanded_widgets_row_count[1] = 0;
			}
			elseif($justlanded_widgets_row_count[1] == 1) {
				$before_widget = str_replace('class="', 'class="widget-container '.$column_class.' first ', $before_widget);
				$params[0]['before_widget'] = $before_widget;
			}
			else {
				$before_widget = str_replace('class="', 'class="widget-container '.$column_class.' ', $before_widget);
				$params[0]['before_widget'] = $before_widget;
			}
		}
		...
		if ($params[0]['id'] == "landing-page-widgets-row-2"){
			...
			$before_widget = $params[0]['before_widget'];
			if ($justlanded_widgets_row_count[2] == $max_columns) {
				$before_widget = str_replace('class="', 'class="widget-container '.$column_class.' last ', $before_widget);
				$params[0]['before_widget'] = $before_widget;
		...
		if ($params[0]['id'] == "footer-widgets"){
			...
			$before_widget = $params[0]['before_widget'];
			if ($justlanded_widgets_row_count[0] == $max_columns) {
				$before_widget = str_replace('class="', 'class="widget-container '.$column_class.' last ', $before_widget);
				$params[0]['before_widget'] = $before_widget;
...
function rgb_hsl($rgb = array(0,0,0)){
...
if(!function_exists('justlanded_content_filters')){
	function justlanded_content_filters($content, $type = "")
	{
		...
		else { // the_content shall be applied to video
...
if (!function_exists('justlanded_set_default_blog_profile')) {
	function justlanded_set_default_blog_profile() {
		if (justlanded_is_blog() && BLOG_DEFAULT_PROFILE != 0) {
			global $data;
			$data = get_option( OPTIONSPREFIX . BLOG_DEFAULT_PROFILE );
		}
	}
}
add_action('wp', 'justlanded_set_default_blog_profile');
