...
jQuery(document).ready(function($) {
    ...
    // link to link
  if($("select[name='dankov_portf_icon'] option:selected").val() == 'link_to_link') {
    $("#dankov_portf_link").show();
  }

    $('select[name="dankov_portf_icon"]').on("change", function(event){
      var str = '';
      str = $("select[name='dankov_portf_icon'] option:selected").val();

     if(str == 'link_to_link')
         $("#dankov_portf_link").fadeIn();
      else
          $("#dankov_portf_link").hide();
       
    }); 

    // link to image gallery lightbox
  if($("select[name='dankov_portf_icon'] option:selected").val() == 'lightbox_to_gallery') {
    $("#dankov_portf_gallery").show();
  }
    ...
    $('select[name="dankov_portf_icon"]').on("change", function(event){
     ...
     if(str == 'lightbox_to_gallery')
         $("#dankov_portf_gallery").fadeIn();
      else
          $("#dankov_portf_gallery").hide();
       
    });   

    // link to video in lightbox
  if($("select[name='dankov_portf_icon'] option:selected").val() == 'lightbox_to_video') {
    $("#dankov_portf_video").show();
  }
    ...
    $('select[name="dankov_portf_icon"]').on("change", function(event){
     ...
     if(str == 'lightbox_to_video')
         $("#dankov_portf_video").fadeIn();
      else
          $("#dankov_portf_video").fadeOut();
       
    });      


  if($("#dankov_portfolio_navigation input[value='no-filter']:checked").val() == 'no-filter') {
    $("#dankov_nav_number").show();
  }  

    $('input[name="dankov_portfolio_navigation"]').on("change", function(event){
      var str = '';
      str = $("#dankov_portfolio_navigation input[value='no-filter']:checked").val();

     if(str == 'no-filter')
         $("#dankov_nav_number").fadeIn();
      else
          $("#dankov_nav_number").fadeOut();
	...
	jQuery.fn.extend({
	    ShowStyles: function () {
			...
			subheader_styles['style-1'] = "#dankov_bg_type, #dankov_subheader_img, #dankov_subheader_color";
			subheader_styles['style-2'] = "#dankov_bg_type, #dankov_subheader_img, #dankov_subheader_color";
			subheader_styles['style-3'] = "#dankov_bg_type, #dankov_subheader_img, #dankov_subheader_color";
	...
	if ((a == ('template-portfolio.php')) || (a == ('template-gallery.php')) || (a == ('template-homepage.php')) || (a == ('template-onepage.php')) ) {
		...
		jQuery('#page_sidebars #dankov_sidebar_position').hide();
	}
	...
	jQuery.fn.extend({
	    ShowPostFormats: function () {
			...
			var post_formats = {};
			post_formats['post-format-0'] = "#dankov_standard_post_custom_fields";
			post_formats['post-format-gallery'] = "#dankov_gallery_post_custom_fields";
			post_formats['post-format-link'] = "#dankov_link_post_custom_fields";
			post_formats['post-format-quote'] = "#dankov_quote_post_custom_fields";
			post_formats['post-format-audio'] = "#dankov_audio_post_custom_fields";
			post_formats['post-format-video'] = "#dankov_video_post_custom_fields";
	...
	jQuery.fn.extend({
	    ShowPageFields: function () {
			...
			//page templates
			var page_templates = {};
			page_templates['template-blog.php'] = "#dankov_blog_options";
			page_templates['template-portfolio.php'] = "#dankov_portfolio_options";
			page_templates['template-contact.php'] = "#dankov_contact_options";
			page_templates['template-onepage.php'] = "#dankov_onepage_options";
